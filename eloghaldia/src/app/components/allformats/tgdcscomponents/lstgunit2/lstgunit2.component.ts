...
import * as _ from 'underscore';
  ...
  @Component({
    selector: 'app-lstgunit2',
    templateUrl: './lstgunit2.component.html',
    providers: [MessageService]
  })
  export class Lstgunit2Component {
    ...
    lstgunit2Arr: any[] = [
      ...
      { parameter: 'EXCITATION VOLTAGE/CURRENT', unit: 'VOLT', max: '45.5', min: 'NA', reading1: "excitation_voltage_1", reading2:  "excitation_voltage_2", reading3:  "excitation_voltage_3" },
      ...
      { parameter: 'DEAERATOR TEMPERATURE', unit: 'DegC', max: '158', min: 'NA', reading1: '', reading2: '', reading3: '' },
      ...
      { parameter: 'TURBINE BEARING VIBRATION (MAX) BRG. NO.', unit: 'MICRON', max: '164', min: 'NA', reading1: 'tbv_brgno_1', reading2: 'tbv_brgno_2', reading3: 'tbv_brgno_3' },
      { parameter: 'TURBINE BEARING TEMP. (MAX) BRG. NO.', unit: 'DegC', max: '95', min: 'NA', reading1: 'tbt_brgno_1', reading2: 'tbt_brgno_2', reading3: 'tbt_brgno_3' },
      ...
      { parameter: 'UNIT 2 TOTAL GENERATION', unit: 'MW', max: '', min: '', reading1: 'shift_gen_1', reading2: 'shift_gen_2', reading3: 'shift_gen_3' },
      { parameter: 'MAXIMUM LOAD', unit: 'MW', max: '', min: '', reading1: this.tgunit2Obj.max_load_1 },
      { parameter: 'MINIMUM LOAD', unit: 'MW', max: '', min: '', reading1: this.tgunit2Obj.min_load_1 },
      { parameter: 'AVERAGE LOAD DURING SHIFT', unit: 'MW', max: '', min: '', reading1: this.tgunit2Obj.avg_load_shift_1 }
    ];
    ...
    ngOnInit(){  
      ...
      this.sharedService.shiftname$.subscribe(data=>{
        this.tgunit2Obj.shiftname = data;
        //alert("name"+data);
        this.fetchdata();
      })
      
      this.sharedService.shiftdate$.subscribe(data=>{
        this.tgunit2Obj.shiftdate = data;
        //alert("date"+data);
        this.fetchdata();
      })
    ...
    insertfunc(){ 
      console.log("date and shift ",this.tgunit2Obj.shiftdate,this.tgunit2Obj.shiftname) ;
      if(_.isEmpty(this.tgunit2Obj.shiftdate)){
        this.tgunit2Obj.shiftdate = this.sharedService.shiftdate;
      }
      if(_.isEmpty(this.tgunit2Obj.shiftname)){
        this.tgunit2Obj.shiftname = this.sharedService.shiftname;
      }
      
      this.tgunit2Obj.shiftdatestr = moment(this.tgunit2Obj.shiftdate).format('YYYY-MM-DD');
      this.httpService.post("inserttgdcslstgunit2",this.tgunit2Obj).subscribe(data => {
    ...
    fetchdata(){
      this.tgunit2Obj.shiftdate = this.sharedService.shiftdate;
      console.log("date and shift ",moment(this.tgunit2Obj.shiftdate).format('YYYY-MM-DD'),this.tgunit2Obj.shiftname)
      let shiftdatestr = moment(this.tgunit2Obj.shiftdate).format('YYYY-MM-DD')
      this.httpService.post("gettgdcslstgunit2",{
        shiftdate :shiftdatestr,
        shiftname:this.tgunit2Obj.shiftname
      }).subscribe(
        {
          next: (data:any) => {
            ...
            if(_.isEmpty(data.data)){
              this.tgunit2Obj = Object.fromEntries(
                Object.keys(this.tgunit2Obj).map((key) => [key, ""]) // Map each key to a blank value
              );
            }else{      
              //Sneha : used mapping to avoid loss of any key which won't come from DB
              this.tgunit2Obj=Object.entries(this.tgunit2Obj).reduce((acc:any, [key, value]) => {
                acc[key] = data.data[key];  // Map the key from pickObject to the accumulator
                return acc;
              }, {});
            }
          },
