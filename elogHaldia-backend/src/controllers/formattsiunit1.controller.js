...
/**
 * Insert data for TSI Unit 1
 */
exports.inserttsiunit1 = async (req, res, next) => {
  const client = await pool.connect();
  try {
    const decryptedBody = await crypto.decipher(req.body.encryptedbody);
    const body = JSON.parse(decryptedBody);
    console.log("Insert TSI Unit 1 body:", body);
    ...
    return res.status(200).json({ success: true, message: "TSI Unit 1 insert successful" });

  } catch (error) {
    console.error("TSI Unit 1 insert failed:", error);
...
/**
 * Get data for TSI Unit 1
 */
exports.gettsiunit1 = async (req, res, next) => {
  const client = await pool.connect();
  try {
    const decryptedBody = await crypto.decipher(req.body.encryptedbody);
    const body = JSON.parse(decryptedBody);
    console.log("gettsiunit1", body);

    const selectquery = `
      SELECT * FROM tgdcs_tsiunit1
    ...
    console.error("Error in gettsiunit1:", error);
